...
def resolve_config_val(val, context_vars):
    """حل کردن متغیرها با اولویت متغیرهای مسیر و سپس محیطی"""
    if not isinstance(val, str): return val
...
def parse_identifier(name_str):
    """تبدیل 'schema.table' به sql.Identifier"""
    if not name_str: return None
    parts = name_str.split('.')
    return sql.Identifier(*parts)

# ==========================================
# توابع زیرساخت امنیتی
# ==========================================
...
def generate_filter_logic(filter_columns):
    """تولید کد PL/pgSQL برای اضافه کردن شرط‌ها."""
    logic_lines = []
    for fc in filter_columns:
        fc_name = fc['name']
        fc_op = fc.get('filter_type', '=')
        fc_type = fc.get('type', 'text').lower()
        
        is_numeric = any(t in fc_type for t in ['int', 'num', 'float', 'double', 'decimal', 'serial', 'real'])

        if is_numeric:
            logic_lines.append(f"""
            IF p_{fc_name} IS NOT NULL THEN
                v_query := v_query || ' AND {fc_name} {fc_op} ' || p_{fc_name}::text;
            END IF;
            """)
        else:
            logic_lines.append(f"""
            IF p_{fc_name} IS NOT NULL THEN
                v_query := v_query || ' AND {fc_name} {fc_op} ' || quote_literal(p_{fc_name});
            END IF;
            """)
...
def create_incremental_job_functions(conn_db, jobs_config, context_vars):
    ...
    for job in jobs_config:
        ...
        func_args_list = [
            f"p_last_{key_column} {key_type_raw} DEFAULT 0",
        ...
        if 'int' in key_type or 'num' in key_type or 'serial' in key_type:
            declare_section = f"""
                ...
                v_query TEXT;
            """
            # 🟢 اصلاح شده: استفاده از RETURN QUERY EXECUTE به جای FOR LOOP برای رفع مشکل HV00L
            logic_section = f"""
            ...
            declare_section = f"""
                ...
                v_query TEXT;
            """
            # 🟢 اصلاح شده: استفاده از RETURN QUERY EXECUTE
            logic_section = f"""
                v_limit := LEAST(COALESCE(p_limit, {batch_size}), {max_limit});
                v_start_val := p_last_{key_column};
...
def create_custom_functions(conn_db, functions_config, context_vars):
    ...
    for func in functions_config:
        ...
        # -------------------------------------------------------
        # حالت ۱: تابع با SQL خام (Raw SQL)
        # -------------------------------------------------------
        if 'sql' in func:
            raw_sql = func['sql']
            raw_sql_resolved = resolve_config_val(raw_sql, context_vars)
            
            # اضافه کردن دستور CREATE FUNCTION در صورت وجود پارامترها
            if 'arguments' in func:
                args_list = [f"{a['name']} {a['type']}" for a in func['arguments']]
                ...
                full_sql = raw_sql_resolved

            try:
                cur.execute(sql.SQL(full_sql))
                print(f"      ⚙️  Created Custom Function (Raw SQL): {func_name_str}")
            except Exception as e:
                print(f"      ❌ Raw SQL Function Error {func_name_str}: {e}")
            continue

        # -------------------------------------------------------
        # حالت ۲: تولید خودکار تابع (Generated)
        # -------------------------------------------------------
        ...
        if not return_columns:
            print(f"      ⚠️ Skipping {func_name_str}: 'return_columns' not defined.")
            continue
            
        # ساخت بخش RETURNS TABLE
        ret_def_list = [f"{rc['name']} {rc['type']}" for rc in return_columns]
        ret_def_sql = ", ".join(ret_def_list)
        
        # ساخت بخش SELECT با Cast صریح
        select_expr_list = [f"{rc['expression']}::{rc['type']} AS {rc['name']}" for rc in return_columns]
        select_expr_sql = ", ".join(select_expr_list)
        
        # ساخت آرگومان‌های ورودی
        ...
        # استفاده از RETURN QUERY EXECUTE برای سازگاری با FDW
        func_sql = f"""
        {drop_sql}
        ...
        # ساخت توضیحات
        param_names = [fc['name'] for fc in filter_columns]
        comment_text = f"Parameters: {', '.join(param_names)}"
        comment_sql = f"COMMENT ON FUNCTION {func_name_str}({drop_types_sql}) IS '{comment_text}';"
        
        try:
            cur.execute(func_sql)
            cur.execute(comment_sql)
            print(f"      ⚙️  Created Custom Function (Generated): {func_name_str}")
        except Exception as e:

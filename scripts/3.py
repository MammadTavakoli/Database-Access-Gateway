...
def create_incremental_job_functions(conn_db, jobs_config, context_vars):
    ...
    for job in jobs_config:
        ...
        # استفاده از حلقه برای بازگشت دادن ردیف‌ها
        execution_block = f"""
            {filter_logic_sql}
...
def create_custom_functions(conn_db, functions_config, context_vars):
    ...
    for func in functions_config:
        ...
        # -------------------------------------------------------
        # حالت ۳: تولید خودکار (Generated) - اصلاح شده برای OUT Parameters
        # -------------------------------------------------------
        ...
        # تولید منطق فیلتر (String Concatenation برای رفع HV00L)
        filter_logic_block = []
        for fc in filter_columns:
            fc_name = fc['name']
            fc_type = fc['type']
        ...
        # 🟢 تولید بلوک انتساب برای OUT Parameters (رفع خطای RETURN NEXT)
        # وقتی RETURNS TABLE داریم، باید متغیرها را منتسب کنیم و RETURN NEXT بدون پارامتر بزنیم
        assign_list = []
        for rc in return_columns:
            # نام متغیر خروجی = نام ستون در کوئری (r.col_name)
            assign_list.append(f"{rc['name']} := r.{rc['name']};")
        ...
        try:
            cur.execute(func_sql)
            print(f"      ⚙️  Created Generated Function (Fixed OUT Params): {func_name_str}")
        except Exception as e:
